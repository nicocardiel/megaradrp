...
def assign_boxes_to_fibers(pseudo_slit_config, insmode):
    ...
    for dumbox in pseudo_slit_config:
        ...
        name = dumbox["name"]
        n2 = n1 + nfibers
        fibid_with_box += [f"{val1}  [{val2}]" for val1, val2 in zip(range(n1, n2), [name] * nfibers)]
...
def plot_trace(ax, center_model, xmin, xmax, ix_offset, rawimage, fibids, fiblabel, colour, correction=0):
    if xmin == xmax == 0:
        num = 4096
...
def main(args=None):
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=float,
        help="Global offset polynomial coefficients " "(+upwards, -downwards)",
    )
    parser.add_argument("--fibids", help="Display fiber identification number", action="store_true")
    parser.add_argument("--verbose", help="Enhance verbosity", action="store_true")
    ...
    if total_fibers != len(fibid_with_box):
        raise ValueError("Mismatch between number of fibers and " "expected number from account from boxes")
